    ...
    public function exportConfigImg(Request $request, $profile = 'ch-1') {
        ...
        if($saveBuildPc) {
            ...
            if(count($listProducts) > 0) {
                ...
                $img->rectangle(0, 0, 1200, 150, function ($draw) {
                ...
                foreach ($listProducts as $index => $product) {
                    ...
                    $img->text(Str::limit($product->name, 120),  340, $yOffset, function($font) {
                        $font->file(storage_path('app/Roboto-Regular.ttf'));
                        $font->size(28);
                        $font->valign('top');
                        $font->color('#000');
                    });
                    $img->text(Helper::formatMoney($product->getPrice()), 340, $yOffset+53, function($font) {
                        $font->file(storage_path('app/Roboto-Regular.ttf'));
                        $font->size(22);
                        $font->valign('top');
                        $font->color('#000');
                    });
                    $img->text(Helper::formatMoney($product->warranty), 360, $yOffset+53, function($font) {
                        $font->file(storage_path('app/Roboto-Regular.ttf'));
                        $font->size(22);
                        $font->valign('top');
                        $font->color('#000');
                    });
                    $img->text(' x ' .Helper::formatMoney($product->quantity, ''), 480, $yOffset+50, function($font) {
                        $font->file(storage_path('app/Roboto-Regular.ttf'));
                        $font->size(28);
                        $font->valign('top');
                        $font->color('#F44336');
