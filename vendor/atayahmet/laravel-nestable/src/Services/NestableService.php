...
class NestableService
{
     ...
     * @var array
     */
    protected $optionUlAttr = ['class' => 'dd-list'];
     ...
     * @return string
     */
    public function renderAsDropdownExt($data = false, $parent = 0, $first = true, $level = 0, $prefixText = '')
    {
        $args = $this->setParameters(func_get_args());
        ...
        $args['data']->each(function ($child_item) use (&$tree, $args, $level, $prefixText) {
            ...
            if (intval($child_item[$this->parent]) == intval($args['parent'])) {
                ...
                // Generating dropdown item
                $childItems .= '<option '.$selected.' value="'.$value.'">'.$levels.$prefix.$prefixText.$label.'</option>';
                ...
                if ($this->hasChild($this->parent, $item_id, $args['data'])) {
                    ...
                    // function call again for child elements
					$prefixText = $prefixText .= "{$label} / ";
                    $childItems .= $this->renderAsDropdownExt($args['data'], $item_id, false, $level, $prefixText);
                }
            }
	...
	public function renderAsMultipleExt()
    {
        return $this->multiple()->renderAsDropdownExt();
    }
